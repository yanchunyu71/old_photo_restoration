...
class UNet(nn.Layer):
    def __init__(
        ...
        self.first = nn.Sequential(
            *[nn.Pad2D(3,mode='reflect'), nn.Conv2D(in_channels, 2 ** wf, kernel_size=7), nn.LeakyReLU(0.2, True)]
        )
        prev_channels = 2 ** wf
        ...
        if with_tanh:
            self.last = nn.Sequential(
                *[nn.Pad2D(1,mode='reflect'), nn.Conv2D(prev_channels, out_channels, kernel_size=3), nn.Tanh()]
            )
        else:
            self.last = nn.Sequential(
                *[nn.Pad2D(1,mode='reflect'), nn.Conv2D(prev_channels, out_channels, kernel_size=3)]
            )
...
class UNetConvBlock(nn.Layer):
    def __init__(self, conv_num, in_size, out_size, padding, batch_norm):
        ...
        for _ in range(conv_num):
            block.append(nn.Pad2D(padding=int(padding),mode='reflect'))
...
class UnetSkipConnectionBlock(nn.Layer):
    ...
    def __init__(
        ...
        if outermost:
            ...
            upconv = nn.Conv2DTranspose(inner_nc, outer_nc, kernel_size=4, stride=2, padding=1, bias=use_bias)
...
if __name__ == "__main__":
    ...
    device = paddle.device("cuda" if paddle.CUDAPlace.is_available() else "cpu")
    ...
    x = paddle.zeros(1, 3, 256, 256).requires_grad_(True).cuda()
    g = make_dot(model(x))
