...
import numpy as np
import random
import paddle.vision.transforms as ptransforms
import paddle
...
def get_transform(opt, params, method=Image.BICUBIC, normalize=True):
    ...
    if 'resize' in opt.resize_or_crop:
        ...
        transform_list.append(ptransforms.Resize(osize, method))   
    ...
    #    transform_list.append(transforms.Lambda(lambda img: __scale_width(img, opt.loadSize, method)))  ## Here , We want the shorter side to match 256, and Scale will finish it.
        transform_list.append(ptransforms.Resize(256,method))
    ...
    if 'crop' in opt.resize_or_crop:
        if opt.isTrain:
            ...
            if opt.test_random_crop:
                transform_list.append(ptransforms.RandomCrop(opt.fineSize))
            else:
                transform_list.append(ptransforms.CenterCrop(opt.fineSize))
    ...
    transform_list += [ptransforms.ToTensor()]

    if normalize:
        transform_list += [ptransforms.Normalize((0.5, 0.5, 0.5),
                                                (0.5, 0.5, 0.5))]
    return ptransforms.Compose(transform_list)

def normalize():    
    return ptransforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))
