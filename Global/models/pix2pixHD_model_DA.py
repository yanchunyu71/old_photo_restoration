...
class Pix2PixHDModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        if self.isTrain:
            ...
            self.optimizer_G = paddle.optimizer.Adam(params, lr=opt.lr, betas=(opt.beta1, 0.999))

            # optimizer D
            params = list(self.netD.parameters())
            self.optimizer_D = paddle.optimizer.Adam(params, lr=opt.lr, betas=(opt.beta1, 0.999))

            params = list(self.feat_D.parameters())
            self.optimizer_featD = paddle.optimizer.Adam(params, lr=opt.lr, betas=(opt.beta1, 0.999))
    ...
    def encode_input(self, label_map, inst_map=None, real_image=None, feat_map=None, infer=False):
        ...
        if self.use_features:
            ...
            if self.opt.load_features:
                feat_map = paddle.fluid.data(feat_map.data.cuda())
    ...
    def forward(self, label, inst, image, feat, infer=False):
        ...
        noise = paddle.optimizer.Adam(torch.randn(hiddens.size()).cuda(hiddens.data.get_device()))
