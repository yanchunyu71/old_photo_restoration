...
def define_G(input_nc, output_nc, ngf, netG, k_size=3, n_downsample_global=3, n_blocks_global=9, n_local_enhancers=1,
    ...
    print(netG)
    #if len(gpu_ids) > 0:
        #assert(paddle.CUDAPlace.is_available())
        #netG.cuda(gpu_ids[0])
...
class NLayerDiscriminator(nn.Layer):
    def __init__(self, input_nc, opt, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2D, use_sigmoid=False, getIntermFeat=False):
        ...
        sequence = [[SN(nn.Conv2D(input_nc, ndf, kernel_size=kw, stride=2, padding=padw),opt.use_SN), nn.LeakyReLU(0.2)]]
        ...
        for n in range(1, n_layers):
            ...
            sequence += [[
                ...
                norm_layer(nf,momentum=0.1), nn.LeakyReLU(0.2)
            ]]
...
class Patch_Attention_4(nn.Layer):  ## While combine the feature map, use conv and mask
    ...
    def Hard_Compose(self, input, dim, index):
        ...
        return paddle.gather(input, dim, index)
    ...
    def forward(self, z, mask):  ## The shape of mask is Batch*1*H*W
        ...
        non_mask_region=(paddle.mean(mask_unfold,dim=1,keepdim=True)>0.6).float()
        ...
        R, max_arg=paddle.max(correlation_matrix,dim=2)
        ...
        concat_1=paddle.concat((z,composed_fold,mask),dim=1)
    ...
    def inference_forward(self,z,mask): ## Reduce the extra memory cost
        ...
        mask_unfold=F.unfold(mask, kernel_size=(self.patch_size,self.patch_size), padding=0, stride=self.patch_size)
        non_mask_region=(paddle.mean(mask_unfold,dim=1,keepdim=True)>0.6).float()[0,0,:] # 1*1*all_patch_num
    ...
    def get_target_tensor(self, input, target_is_real):
        ...
        if target_is_real:
            ...
            if create_label:
                ...
                #self.real_label_var = Variable(real_tensor, requires_grad=False) #reuqires_grad=True来建立一个反向传播图，从而能够计算梯度
