...
class Pix2PixHDModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        if self.isTrain:
            ...
            self.optimizer_G = paddle.optimizer.Adam(learning_rate=opt.lr,beta1=opt.beta1,beta2=0.999,parameters=params)                            

            # optimizer D                        
            params = list(self.netD.parameters())    
            self.optimizer_D = paddle.optimizer.Adam(learning_rate=opt.lr,beta1=opt.beta1,beta2=0.999,parameters=params)
    ...
    def encode_input(self, label_map, inst_map=None, real_image=None, feat_map=None, infer=False):             
        if self.opt.label_nc == 0:
            ...
            #input_label = torch.cuda.FloatTensor(torch.Size(oneHot_size)).zero_() # 创建一个torch.size大的全0的tensor
            input_label = paddle.zeros(oneHot_size)
        ...
        if not self.opt.no_instance:
            ...
            input_label = paddle.concat((input_label, edge_map), dim=1)         
    ...
    def discriminate(self, input_label, test_image, use_pool=False):
        if input_label is None:
            ...
            input_concat = paddle.concat((input_label, test_image.detach()), dim=1)
        if use_pool:            
    ...
    def forward(self, label, inst, image, feat, infer=False):
        ...
        if self.use_features:
            ...
            input_concat = paddle.concat((input_label, feat_map), dim=1)                        
        else:
        ...
        if self.opt.no_cgan:
            ...
            # GAN loss (Fake Passability Loss)        
            pred_fake = self.netD.forward(paddle.concat((input_label, fake_image), dim=1))        
            loss_G_GAN = self.criterionGAN(pred_fake, True) 
        
        
        loss_G_kl = paddle.mean(torch.pow(hiddens, 2)) * self.opt.kl
    ...
    def get_edges(self, t):
        ...
        edge = paddle.zeros(t.size(),dtype='int32')
    ...
    def update_fixed_params(self):
        ...
        if self.gen_features:
            params += list(self.netE.parameters())           
        #self.optimizer_G = paddle.optimizer.Adam(params, lr=self.opt.lr, betas=(self.opt.beta1, 0.999))
        delf.optimizer_G = paddle.optimizer.Adam(learning_rate=self.opt.lr,beta1=self.opt.beta1,beta2=0.999,parameters=params)
        if self.opt.verbose:
