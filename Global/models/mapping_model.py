...
class Pix2PixHDModel_Mapping(BaseModel):
    ...
    def initialize(self, opt):
        ...
        if self.isTrain:
            ...
            if not opt.no_load_VAE:
                ...
                self.optimizer_mapping = paddle.optim.Adam(params, lr=G_lr, betas=(beta1, beta2))

            # optimizer D                        
            params = list(self.netD.parameters())    
            self.optimizer_D = paddle.optimizer.Adam(params, lr=D_lr, betas=(beta1, beta2))
    ...
    def encode_input(self, label_map, inst_map=None, real_image=None, feat_map=None, infer=False):             
        if self.opt.label_nc == 0:
            ...
            input_label = paddle.cuda.FloatTensor(paddle.Size(oneHot_size)).zero_()
        ...
        input_label = paddle.to_tensor(input_label, stop_gradient=infer)      
        #input_label = paddle.fluid.data(input_label, volatile=infer)

        # real images for training
        if real_image is not None:
            real_image = paddle.fluid.data(real_image.data.cuda())
    ...
    def forward(self, label, inst, image, feat, pair=True, infer=False, last_label=None, last_image=None):
        ...
        if self.opt.feat_gan:
            ...
            pred_fake = self.netD.forward(paddle.concat((label_feat.detach(), label_feat_map), dim=1))        
