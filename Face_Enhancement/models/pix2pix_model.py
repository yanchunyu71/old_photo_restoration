...
class Pix2PixModel(paddle.nn.Layer):
    ...
    def create_optimizers(self, opt):
        ...
        optimizer_G = paddle.optimizer.Adam(G_params, lr=G_lr, betas=(beta1, beta2))
        optimizer_D = paddle.optimizer.Adam(D_params, lr=D_lr, betas=(beta1, beta2))
    ...
    def discriminate(self, input_semantics, fake_image, real_image):
        ...
        if self.opt.no_parsing_map:
            ...
            fake_concat = paddle.concat([input_semantics, fake_image], dim=1)
            real_concat = paddle.concat([input_semantics, real_image], dim=1)

        # In Batch Normalization, the fake and real images are
        # recommended to be in the same batch to avoid disparate
        # statistics in fake and real images.
        # So both fake and real images are fed to D all at once.
        fake_and_real = paddle.concat([fake_concat, real_concat], dim=0)
    ...
    def reparameterize(self, mu, logvar):
        ...
        eps = paddle.rand(std.size(),dtype=std.dtype)
